'''
   import data from beancount to pandas
'''
                ...
                for item_no in range(column_index_tuple[col]):
                    if item_no == 0:
                        ...
                        if col not in indexes_done:
                            col_names.insert(col+item_no,'{}_{}'.format(col_names[col][slice(0,3)],item_no))
...
#        if max_n_values > 1:
    ...
    def _convert_from_bean(self, val, col_type):
        ...
        elif col_type is beancount.core.amount.Amount:
            amount, curr = val
            return (float(amount), curr)
